...
void AnalogTapeModule::reset()
{
    ...
    wowPhase = 0.0f;
    flutterPhase = 0.0f;
    transportPhaseRadians = 0.0f;
    telemetryState = {};
    telemetry.clear();
    initialized = false;
}

bool AnalogTapeModule::readContinuousTelemetry(
                               ...
                               const ProcessEnvironment& environment)
{
    ...
    const auto machine = discreteIndex(normalized(0, 0.375f), machineCount);
    const auto speed = discreteIndex(normalized(4, 0.625f), speedCount);
    ...
    const auto inputDb = lerp(-18.0f, 18.0f, normalized(1, 0.5f));
    const auto inputTarget = std::abs(inputDb) < 0.0001f
        ? 1.0f : juce::Decibels::decibelsToGain(inputDb);
    const auto driveTarget = normalized(2, 0.30f);
    const auto biasTarget = lerp(-1.0f, 1.0f, normalized(3, 0.5f));
    ...
    const auto headBumpTarget = normalized(5, 0.35f);
    const auto wowTarget = normalized(6, 0.20f);
    const auto flutterTarget = normalized(7, 0.20f);
    const auto wearTarget = normalized(8, 0.15f);
    const auto noiseTarget = normalized(9, 0.20f);
    const auto mixTarget = normalized(10, 1.0f);
    const auto outputDb = lerp(-18.0f, 12.0f, normalized(11, 0.60f));
    ...
    float lastModulationMilliseconds = 0.0f;
    float minimumDropoutGain = 1.0f;
    float maximumEnvelope = 0.0f;
    static constexpr std::array<float, speedCount> transportRates {
        0.12f, 0.22f, 0.35f, 0.60f
    };
    for (int sample = 0; sample < numSamples; ++sample)
    {
        ...
        lastModulationMilliseconds =
            modulationSamples * 1000.0f / static_cast<float>(sampleRate);
        transportPhaseRadians += juce::MathConstants<float>::twoPi
            * transportRates[static_cast<size_t>(speed)]
            / static_cast<float>(sampleRate);
        if (transportPhaseRadians >= juce::MathConstants<float>::twoPi)
            transportPhaseRadians -= juce::MathConstants<float>::twoPi;
        ...
        for (int channel = 0; channel < buffer.getNumChannels(); ++channel)
        {
            ...
            minimumDropoutGain = juce::jmin(minimumDropoutGain, dropoutGain);
            maximumEnvelope = juce::jmax(maximumEnvelope, gate);
    ...
    if (environment.captureTelemetry)
    {
        ...
        telemetryState.values[transportPhase] =
            transportPhaseRadians / juce::MathConstants<float>::twoPi;
        telemetryState.values[modulationMilliseconds] =
            lastModulationMilliseconds;
        telemetryState.values[currentDropoutGain] = minimumDropoutGain;
        telemetryState.values[signalEnvelope] = maximumEnvelope;
