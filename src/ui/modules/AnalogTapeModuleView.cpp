    ...
    juce::Component& component) const
{
    ...
    component.setHelpText(
        "Choose a machine with the top pills. The reel graphic follows the "
        "DSP transport at the selected Tape Speed; drag the speed "
    ...
    juce::Graphics& graphics, juce::Rectangle<float> area)
{
    ...
    const auto hasTelemetry = readContinuousTelemetry(telemetry)
        && telemetry.valueCount >= AnalogTapeModule::telemetryValueCount;
    const auto reelAngle = hasTelemetry
        ? telemetry.values[AnalogTapeModule::transportPhase]
